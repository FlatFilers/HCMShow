...
export const roundCurrencyValues = (
  ...
  fields.forEach((field) => {
    try {
      ...
      if (
        !isNaN(parseFloat(fieldValue as string)) &&
        isFinite(fieldValue as number)
      ) {
        // Convert the field value to a numeric value and round it to the nearest 2 decimal places
        const roundedValue = parseFloat(fieldValue as string).toFixed(2);
      ...
      record.addError(
        field,
        `An error occurred while rounding: ${(err as Error).message}`
      );
    }
  });
